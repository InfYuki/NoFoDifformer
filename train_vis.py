...
# 新增：保存最终滤波器响应（放在文件靠前位置的工具函数区域）
from scipy.io import savemat  # 顶部需导入
def save_final_filter_response(net, eigvals, save_dir, run_id, sample_points=512):
    """
    保存训练完成后的滤波器响应（同时保存 npz 和 mat）。
    """
    os.makedirs(save_dir, exist_ok=True)
    device = next(net.parameters()).device
    net.eval()

    e_min, e_max = eigvals.min().item(), eigvals.max().item()
    e_grid = torch.linspace(e_min, e_max, steps=sample_points, device=device)

    with torch.no_grad():
        per_channel = net.eig_encoder(e_grid)
        final_resp = net.alpha(torch.cat(per_channel, dim=1))

        freqs_all_layers = []
        if hasattr(net, "eig_encoder") and hasattr(net.eig_encoder, "_construct_freqs"):
            for l in range(net.eig_encoder.num_layers):
                freqs_all_layers.append(net.eig_encoder._construct_freqs(l).detach().cpu().numpy())

        weights = [
            [lin.weight.detach().cpu().numpy() for lin in net.eig_encoder.readouts_layers[l]]
            for l in range(net.eig_encoder.num_layers)
        ]
        biases = [
            [lin.bias.detach().cpu().numpy() for lin in net.eig_encoder.readouts_layers[l]]
            for l in range(net.eig_encoder.num_layers)
        ]

    # 保存 npz（可选）
    npz_path = os.path.join(save_dir, f"filter_response_run{run_id}.npz")
    np.savez(
        npz_path,
        e_grid=e_grid.cpu().numpy(),
        per_channel=[t.detach().cpu().numpy() for t in per_channel],
        final_response=final_resp.detach().cpu().numpy(),
        freqs=freqs_all_layers,
        weights=weights,
        biases=biases,
    )

    # 保存 mat（纯数值，避免 dtype=object）
    per_channel_mat = np.concatenate(
        [t.detach().cpu().numpy() for t in per_channel], axis=1
    )  # 形状: [sample_points, k]
    mat_path = os.path.join(save_dir, f"filter_response_run{run_id}.mat")
    savemat(mat_path, {
        "e_grid": e_grid.cpu().numpy().reshape(-1),
        "final_response": final_resp.detach().cpu().numpy().reshape(-1),
        "per_channel": per_channel_mat,  # 每列是一个通道
        # 如需查看频率/权重，可保留为 cell，体积不大一般没问题：
        "freqs": np.array(freqs_all_layers, dtype=object),
        "weights": np.array(weights, dtype=object),
        "biases": np.array(biases, dtype=object),
    })

    print(f"[run {run_id}] 已保存滤波器响应 npz: {npz_path}")
    print(f"[run {run_id}] 已保存滤波器响应 mat: {mat_path}")

def visualize_frequency_learning(model, epochs_to_visualize, save_dir='./freq_analysis'):
    """Record frequency distribution changes during training"""
    os.makedirs(save_dir, exist_ok=True)

    # 创建图表
    fig, axes = plt.subplots(len(epochs_to_visualize), 1, figsize=(10, 3 * len(epochs_to_visualize)))
    if len(epochs_to_visualize) == 1:
        axes = [axes]

    for i, epoch in enumerate(epochs_to_visualize):
        # 获取频率分布
        if hasattr(model, 'eig_encoder') and hasattr(model.eig_encoder, '_construct_freqs'):
            freqs = model.eig_encoder._construct_freqs(0).detach().cpu().numpy()

            # 绘制频率分布直方图
            axes[i].hist(freqs, bins=20)
            axes[i].set_title(f"Epoch {epoch} Frequency Distribution")
            axes[i].set_xlabel("Frequency")
            axes[i].set_ylabel("Count")

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "frequency_evolution.png"))
    plt.close()
    return freqs


def analyze_periodicity(model, e, u, x, save_dir='./freq_analysis'):
    """Analyze the periodicity of model processing graph data"""
    os.makedirs(save_dir, exist_ok=True)
    device = x.device
    ...
    if hasattr(model, 'eig_encoder') and hasattr(model.eig_encoder, '_construct_freqs'):
        ...
        print("Model does not contain accessible frequency information")
        return

    # 计算不同特征值下的模型输出
    outputs = []
    # 根据当前数据特征值范围创建测试值
    max_eig_val = 4.0  # 假设特征值范围，可以根据实际情况调整
    test_values = np.linspace(0, max_eig_val, 200)

    for val in test_values:
        # 替换特征值进行前向传播
        test_tensor = torch.tensor([val], device=device, dtype=torch.float32)
        with torch.no_grad():
            out = model.eig_encoder(test_tensor)
            # 处理输出格式 - 适应您模型的输出形式
            if isinstance(out, list):
                processed_out = torch.cat(out, dim=1).mean().item()
            else:
                processed_out = out.mean().item()
            outputs.append(processed_out)

    # 可视化输出曲线
    plt.figure(figsize=(10, 6))
    plt.plot(test_values, outputs)
    plt.title("Model Response Curve - Non-periodicity Analysis")
    plt.xlabel("Eigenvalue")
    plt.ylabel("Model Output")
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, "periodicity_analysis.png"))

    # 计算自相关函数检测周期性
    correlation = signal.correlate(outputs, outputs, mode='full')
    correlation = correlation[len(correlation) // 2:]
    plt.figure(figsize=(10, 6))
    plt.plot(correlation)
    plt.title("Autocorrelation Function - Periodicity Detection")
    plt.xlabel("Lag")
    plt.ylabel("Autocorrelation")
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, "autocorrelation.png"))

    # 保存数据以便后续分析
    np.savetxt(os.path.join(save_dir, "freqs.csv"), freqs)
    np.savetxt(os.path.join(save_dir, "model_response.csv"), np.column_stack((test_values, outputs)))

    return freqs, test_values, outputs, correlation
...
def main_worker(args, run_id=0):
    ...
    for idx in range(args.epoch):
        ...
        # 在函数结尾返回前添加以下代码，保存最终频率
        # 创建run特定的目录
        run_dir = f'./freq_analysis_2/{args.dataset}/run_{run_id}'
        os.makedirs(run_dir, exist_ok=True)

        # 保存最终频率
        if hasattr(net, 'eig_encoder') and hasattr(net.eig_encoder, '_construct_freqs'):
            # 以第 0 层频率为例，若有多层可循环
            final_freqs = net.eig_encoder._construct_freqs(0).detach().cpu().numpy()
            np.savetxt(os.path.join(run_dir, "final_freqs.csv"), final_freqs, fmt="%.8f")

            # 可视化单个run的最终频率分布
            plt.figure(figsize=(8, 6))
            plt.hist(final_freqs, bins=20)
            plt.title(f"Run {run_id} Final Frequency Distribution")
            plt.xlabel("Frequency Value")
            plt.ylabel("Count")
            plt.savefig(os.path.join(run_dir, "final_freq_distribution.png"))
            plt.close()
    ...
    # 训练结束后，保存最终滤波器响应（只保存一次）
    run_dir = f'./freq_analysis_2/{args.dataset}/run_{run_id}'
    save_final_filter_response(net, e, save_dir=run_dir, run_id=run_id, sample_points=512)
...
if __name__ == '__main__':
    ...
    for RP in tqdm(range(args.runs)):
        args.seed = SEEDS[RP]
        set_seed(args.seed)
        best_val_acc, best_test_acc, time_run = main_worker(args, run_id=RP)
    ...
    # 完全替换第440-464行的频率分析汇总部分
    # 对所有运行的结果进行频率分析汇总
    if args.runs > 1:
        print("\nStarting summary of frequency analysis results...")
        try:
            # 收集所有运行的最终频率数据
            all_freqs = []
            all_labels = []
            ...
            for run in range(args.runs):
                freq_file = f'./freq_analysis_2/run_{run}/final_freqs.csv'
                if os.path.exists(freq_file):
                    freqs = np.loadtxt(freq_file)
                    all_freqs.append(freqs)
                    all_labels.append(f"Run {run + 1}")
            ...
            if len(all_freqs) > 0:
                ...
                for i, freqs in enumerate(all_freqs):
                    for f in freqs:
                        data.append({'Run': all_labels[i], 'Frequency': f})
                ...
                # 创建小提琴图
                ax = sns.violinplot(x='Run', y='Frequency', data=df)
                plt.title("Frequency Distribution Comparison Across All Runs")
                plt.grid(True)
                plt.savefig('./freq_analysis/all_runs_frequency_distribution_violin.png')
                plt.close()

                # 创建箱线图
                plt.figure(figsize=(15, 10))
                ax = sns.boxplot(x='Run', y='Frequency', data=df)
                plt.title("Frequency Distribution Box Plot Across All Runs")
                plt.grid(True)
                plt.savefig('./freq_analysis/all_runs_frequency_distribution_box.png')
                plt.close()

                # 创建直方图叠加图
                plt.figure(figsize=(15, 8))
                for i, freqs in enumerate(all_freqs):
                    plt.hist(freqs, bins=20, alpha=0.4, label=all_labels[i])
                plt.title("Frequency Distribution Histograms Across All Runs")
                plt.xlabel("Frequency Value")
                plt.ylabel("Count")
                plt.legend()
                plt.grid(True)
                plt.savefig('./freq_analysis/all_runs_frequency_distribution_hist.png')
                plt.close()
                ...
                stats_df.to_csv('./freq_analysis/frequency_stats_summary.csv', index=False)
                print("Frequency analysis summary completed!")


        except Exception as e:
            print(f"Error in summarizing frequency analysis: {e}")
