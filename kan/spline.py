...
def curve2coef(x_eval, y_eval, grid, k):
    ...
    try:
        #print(f"mat shape: {mat.shape}, y_eval shape: {y_eval.shape}")
        #print(f"mat stats - min: {mat.min()}, max: {mat.max()}, mean: {mat.mean()}")
        #coef = torch.linalg.lstsq(mat, y_eval).solution[:, :, :, 0]
        # 在kan/spline.py中的curve2coef函数中
        # 直接使用手动伪逆计算方法，避免使用torch.linalg.lstsq
        lamb = 1e-8  # 正则化参数
        XtX = torch.einsum('ijmn,ijnp->ijmp', mat.permute(0, 1, 3, 2), mat)
        Xty = torch.einsum('ijmn,ijnp->ijmp', mat.permute(0, 1, 3, 2), y_eval)
        n1, n2, n = XtX.shape[0], XtX.shape[1], XtX.shape[2]
        identity = torch.eye(n, n, device=device)[None, None, :, :].expand(n1, n2, n, n)
        A = XtX + lamb * identity  # 添加正则化以提高稳定性
        B = Xty
        coef = (A.pinverse() @ B)[:, :, :, 0]  # 使用伪逆求解
