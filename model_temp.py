...
class NoFoDifformer_temp(nn.Module):
    """
...
# ========================= 图分类模型 =========================
class NoFoDifformer_temp_GraphClassification(nn.Module):
    """
    基于NoFoDifformer_temp的图分类模型
    - 在节点表示基础上添加图级别池化
    - 支持多种池化方法：mean, max, sum, attention
    """

    def __init__(self, nclass, nfeat, nlayer=1, hidden_dim=128, dim=32, nheads=1, k=10,
                 num_layers=1, num_freq=None, Omega=50.0, delta_min=0.25, weight_penalty=1e-4,
                 residual=True, tran_dropout=0.0, feat_dropout=0.0, prop_dropout=0.0,
                 norm='none', pool_method='mean', graph_dropout=0.5):
        """
        Args:
            pool_method: 图级别池化方法，可选 'mean', 'max', 'sum', 'attention'
            graph_dropout: 图分类器中的dropout率
            其他参数与NoFoDifformer_temp相同
        """
        super().__init__()

        # 创建基础节点分类模型
        self.base_model = NoFoDifformer_temp(
            nclass=nclass,
            nfeat=nfeat,
            nlayer=nlayer,
    ...
    def pool_nodes(self, node_repr, mask=None):
        """
        图级别池化操作
        Args:
            node_repr: 节点表示 [N, D] 或 [B, max_N, D] (批处理模式)
            mask: 节点mask [N] 或 [B, max_N] (批处理模式)，True表示真实节点
        Returns:
            graph_repr: 图级别表示 [D] 或 [B, D]
        """
        if len(node_repr.shape) == 2:
            # 单图模式 [N, D]
            if mask is not None:
                # 只对真实节点进行池化
            ...
            if self.pool_method == 'mean':
                ...
                graph_repr = node_repr.max(dim=0)[0]
            elif self.pool_method == 'sum':
                graph_repr = node_repr.sum(dim=0)
            elif self.pool_method == 'attention':
                # 注意力池化
                att_weights = self.attention_pool(node_repr)  # [N, 1]
                att_weights = F.softmax(att_weights, dim=0)
                graph_repr = (node_repr * att_weights).sum(dim=0)
            else:
            ...
            # 批处理模式 [B, max_N, D]
            B, max_N, D = node_repr.shape
            graph_reprs = []

            for i in range(B):
                node_i = node_repr[i]  # [max_N, D]
                mask_i = mask[i] if mask is not None else None

                if mask_i is not None:
                    # 只对真实节点进行池化
                    num_real_nodes = mask_i.sum().item()
                    node_i = node_i[:num_real_nodes]

                if self.pool_method == 'mean':
                    graph_repr_i = node_i.mean(dim=0)
                elif self.pool_method == 'max':
                    graph_repr_i = node_i.max(dim=0)[0]
                elif self.pool_method == 'sum':
                    graph_repr_i = node_i.sum(dim=0)
                elif self.pool_method == 'attention':
                    # 注意力池化
                    att_weights = self.attention_pool(node_i)  # [num_real_nodes, 1]
                    att_weights = F.softmax(att_weights, dim=0)
                    graph_repr_i = (node_i * att_weights).sum(dim=0)
                else:
                    raise ValueError(f"Unknown pool method: {self.pool_method}")

                graph_reprs.append(graph_repr_i)

            return torch.stack(graph_reprs)  # [B, D]

    def forward(self, e, u, x, mask=None):
        """
        前向传播 - 优化版本，支持批量并行处理
        """
        device = next(self.parameters()).device
        batch_mode = len(e.shape) > 1 and e.shape[0] > 1
        ...
        if batch_mode:
            ...
            # 仍然需要逐个处理（因为每个图的节点数不同），但可以减少开销
            for i in range(B):
                mask_i = mask[i] if mask is not None else None
                if mask_i is not None:
                    num_real_nodes = mask_i.sum().item()
                    e_i = e[i, :num_real_nodes].contiguous().to(device)
                    u_i = u[i, :num_real_nodes, :num_real_nodes].contiguous().to(device)
                    x_i = x[i, :num_real_nodes].contiguous().to(device)
                else:
                    # 如果没有mask，需要找到实际的节点数
                    # 假设所有节点都是有效的
                    e_i = e[i].contiguous().to(device)
                    u_i = u[i].contiguous().to(device)
                    x_i = x[i].contiguous().to(device)

                # 通过基础模型得到节点表示
                with torch.cuda.amp.autocast(enabled=False):  # 确保精度
                    node_repr = self.base_model(e_i, u_i, x_i)

                all_node_reprs.append(node_repr)
                if mask_i is not None:
                    all_masks.append(mask_i[:node_repr.shape[0]])

            # 批量池化（如果可能）
            graph_reprs = []
            for i, node_repr in enumerate(all_node_reprs):
                graph_repr = self.pool_nodes(node_repr, mask=None)
                graph_reprs.append(graph_repr.to(device))

            graph_reprs = torch.stack(graph_reprs).to(device)
        else:
            # 单图模式
            e = e.to(device)
            u = u.to(device)
            ...
            if mask is not None:
                ...
                e = e[:num_real_nodes].contiguous()
                u = u[:num_real_nodes, :num_real_nodes].contiguous()
                x = x[:num_real_nodes].contiguous()

            node_repr = self.base_model(e, u, x).to(device)
            graph_reprs = self.pool_nodes(node_repr, mask=None).to(device)

            if len(graph_reprs.shape) == 1:
                graph_reprs = graph_reprs.unsqueeze(0)

        # 图级别分类
        graph_logits = self.graph_classifier(graph_reprs.to(device))

        if not batch_mode and graph_logits.shape[0] == 1:
            graph_logits = graph_logits.squeeze(0)

        return graph_logits.to(device)

    def get_node_representations(self, e, u, x, mask=None):
        """
        获取节点表示（用于可视化或分析）
        Args:
            参数与forward相同
        Returns:
            node_repr: 节点表示 [N, node_dim] 或 [B, max_N, node_dim]
        """
        # 获取模型所在的设备
        device = next(self.parameters()).device
        ...
        if batch_mode:
            ...
            for i in range(B):
                ...
                if mask_i is not None:
                    num_real_nodes = mask_i.sum().item()
                    e_i = e_i[:num_real_nodes].contiguous()
                    u_i = u_i[:num_real_nodes, :num_real_nodes].contiguous()
                    x_i = x_i[:num_real_nodes].contiguous()
            ...
            node_repr = self.base_model(e, u, x)
            return node_repr